...
from electrum_boli import Wallet
...
from electrum_boli.plugins import always_hook, run_hook
from electrum_boli.mnemonic import prepare_seed
...
class InstallWizard(QDialog):
    ...
    def __init__(self, config, network, storage, app):
        ...
        self.setWindowTitle('Electrum-BOLI' + '  -  ' + _('Install Wizard'))
    ...
    def waiting_dialog(self, task, msg= _("Electrum-BOLI is generating your addresses, please wait.")):
        def target():
            task()
    ...
    def network_dialog(self):
        ...
        label = QLabel(_("Electrum-BOLI communicates with remote servers to get information about your transactions and addresses. The servers all fulfil the same purpose only differing in hardware. In most cases you simply want to let Electrum-BOLI pick one at random if you have a preference though feel free to select a server manually.") + "\n\n" \
    ...
    def run(self, action, wallet_type):
        ...
        if action in ['create', 'restore']:
            if wallet_type == 'multisig':
                ...
                hardware_wallets = map(lambda x:(x[1],x[2]), filter(lambda x:x[0]=='hardware', electrum_boli.wallet.wallet_types))
