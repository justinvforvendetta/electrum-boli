...
def hash_160_to_bc_address(h160, addrtype = 85):
...
def bc_address_to_hash_160(addr):
    bytes = base_decode(addr, 5, base=58)
...
def SecretToASecret(secret, compressed=False, addrtype=85):
    vchIn = chr((addrtype+128)&255) + secret
    if compressed: vchIn += '\01'
    return EncodeBase58Check(vchIn)

def ASecretToSecret(key, addrtype=85):
