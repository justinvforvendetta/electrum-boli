...
class Commands:
    ...
    @command('')
    def version(self):
        """Return the version of electrum-boli."""
        import electrum_boli  # Needs to stay here to prevent ciruclar imports
        return electrum_boli.ELECTRUM_VERSION
    ...
    def setlabel(self, key, label):
        """Assign a label to an item. Item may be a BOLI address or a
        transaction ID"""
    ...
    def _format_request(self, out):
        ...
        out['amount'] = format_satoshis(out.get('amount')) + ' BOLI'
...
param_descriptions = {
    ...
    'destination': 'BOLI address, contact or alias',
    'address': 'BOLI address',
    'seed': 'Seed phrase',
    'txid': 'Transaction ID',
    'pos': 'Position',
    'height': 'Block height',
    ...
    'amount': 'Amount to be sent (in BOLI). Type \'!\' to send the maximum available.',
    'requested_amount': 'Requested amount (in BOLI).',
    'csv_file': 'CSV file of recipient, amount',
}

command_options = {
    'broadcast':   (None, "--broadcast",   "Broadcast the transaction to the BOLI network"),
    ...
    'tx_fee':      ("-f", "--fee",         "Transaction fee (in BOLI)"),
...
config_variables = {
    ...
    'addrequest': {
        ...
        'url_rewrite': 'Parameters passed to str.replace(), in order to create the r= part of boli: URIs. Example: \"(\'file:///var/www/\',\'https://electrum.org/\')\"',
    },
    'listrequests':{
        'url_rewrite': 'Parameters passed to str.replace(), in order to create the r= part of boli: URIs. Example: \"(\'file:///var/www/\',\'https://electrum.org/\')\"',
    }
